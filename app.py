...
st.caption("Upload your dataset → deep visuals + statistical tests + recommendations.")
...
if uploaded is None:
    st.info("Upload the dataset CSV to start.")
    st.stop()
...
auto_country = pick_col(df_raw, ["country", "ship_country", "billing_country", "destination_country"])
...
df = df_raw.copy()
df[country_col] = _clean_str(df[country_col])
...
c5.metric("Anchor market", f"{anchor_country}", help=f"Top country share: {anchor_share:.1%}" if pd.notna(anchor_share) else None)

st.divider()

tab_overview, tab_geo, tab_channel, tab_time, tab_stats, tab_data = st.tabs(
    ["Overview", "Geography", "Channels", "Time", "Stats", "Data"]
)
...
with tab_overview:
    ...
    if pd.notna(anchor_share) and anchor_share >= 0.5:
        ...
        recs.append("De-risk by scaling 2–3 mid-tier countries via their locally winning channels.")
    else:
...
with tab_geo:
    ...
    st.markdown("**Insight:** Shows where performance is concentrated.\n\n"
                "**Recommendation:** Protect anchor markets operationally; grow mid-tier markets using the best-fit channel per country.")
    ...
    st.markdown(f"**Insight:** Top {min(5, len(country_totals))} countries contribute ~**{t5:.1%}** of total.\n\n"
                "**Recommendation:** If concentration is high, set explicit growth targets for secondary markets and monitor their channel performance.")
...
with tab_channel:
    ...
    st.markdown("**Insight:** Identifies the channel(s) driving most value.\n\n"
                "**Recommendation:** Invest in the strongest channel where it already wins; avoid forcing identical strategy across countries.")
    ...
    st.markdown("**Insight:** Reveals dominant channels per market.\n\n"
                "**Recommendation:** Build a per-market channel playbook (online-first vs partner-first) based on what already performs locally.")
    ...
    st.markdown("**Insight:** Countries show different channel dependence.\n\n"
                "**Recommendation:** Optimize the top channel in each country first, then test a second channel to diversify within that market.")
    ...
    st.markdown("**Insight:** Shows portfolio structure at a glance.\n\n"
                "**Recommendation:** Use this to pick 2–3 highest-leverage market/channel combos to improve first.")
    ...
    st.markdown("**Insight:** Highlights volatility/outliers by channel.\n\n"
                "**Recommendation:** For high-variance channels, add forecasting buffers and set guardrails (order thresholds, pricing rules).")
...
with tab_time:
    ...
    if date_col == "(none)" or not f[date_col].notna().any():
        ...
        st.markdown("**Insight:** Shows trend + spikes/dips.\n\n"
                    "**Recommendation:** Investigate spikes/drops by filtering to specific countries/channels.")
        ...
        st.markdown("**Insight:** Identifies which markets are growing vs declining.\n\n"
                    "**Recommendation:** Allocate growth investment to consistently upward-trending markets; diagnose declines operationally.")
...
with tab_stats:
    ...
    if ct.shape[0] < 2 or ct.shape[1] < 2:
        ...
        if p < 0.05:
            ...
            st.info("No strong evidence that channel mix differs by country (within current filtered subset).")
        st.markdown("**Recommendation:** If significant (and V is non-trivial), use market-specific channel strategies.")
    ...
    if len(g2) < 2:
        ...
        st.markdown("**Recommendation:** If significant, don’t assume one-size-fits-all pricing/volume behavior across countries; tailor targets and channel focus per market tier.")
